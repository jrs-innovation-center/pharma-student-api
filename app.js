...
app.get('/medications', function(req, res, next) {
	if (
		req.query.filter &&
		split(':', req.query.filter)[0].toLowerCase() === 'ingredient'
	) {
		...
		req.query.filter &&
		split(':', req.query.filter)[0].toLowerCase() === 'form'
	) {
		const result = split(':', req.query.filter)
...
app.get('/patients', function(req, res, next) {
	if (
		req.query.filter &&
		split(':', req.query.filter)[0].toLowerCase() === 'lastname'
	) {
		const result = split(':', req.query.filter)
		...
		req.query.filter &&
		split(':', req.query.filter)[0].toLowerCase() === 'condition'
	) {
		const result = split(':', req.query.filter)
...
app.get('/pharmacies', function(req, res, next) {
	if (
		req.query.filter &&
		split(':', req.query.filter)[0].toLowerCase() === 'chainname'
	) {
		const result = split(':', req.query.filter)
		...
		req.query.filter &&
		split(':', req.query.filter)[0].toLowerCase() === 'storename'
	) {
		const result = split(':', req.query.filter)
