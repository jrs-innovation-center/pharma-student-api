...
function listMedsByLabel(startKey, limit, cb) {
	...
	const meds = startKey
		? compose(drop(1), map(x => x.doc), map(addSortToken))
		: compose(map(x => x.doc), map(addSortToken))
...
function listPharmaciesByStoreName(storeName, cb) {
	db.query(
		'pharmacyByStoreName',
		{ include_docs: true, keys: [storeName] },
		function(err, store) {
			if (err) return cb(err)
			cb(null, store.rows)
		}
...
function listPharmacies(startKey, limit, cb) {
	...
	db.query('pharmacies', options, function(err, list) {
		if (err) return cb(err)

		var mappedQueryResults = shouldWeDrop
			? compose(drop(1), map(returnDoc), map(addSortToken))(list.rows)
			: compose(map(returnDoc), map(addSortToken))(list.rows)
...
function preppedNewPharmacy(doc) {
	var newId =
		'pharmacy_' +
		doc.storeChainName.toLowerCase() +
		'_' +
		doc.storeName.toLowerCase() +
		'_' +
		doc.storeNumber
...
function checkRequiredInputs(doc) {
	return (
		prop('storeNumber', doc) &&
		prop('storeChainName', doc) &&
		prop('storeName', doc) &&
